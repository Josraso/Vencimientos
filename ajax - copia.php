<?php
// ajax.php - Manejador de peticiones AJAX mejorado - Fase 1 y 2
...
try {
    switch ($action) {
        case 'reset_database':
    $clientes = $_POST['clientes'] === 'true';
    $servicios = $_POST['servicios'] === 'true';
    $servicios_cliente = $_POST['servicios_cliente'] === 'true';
    $albaranes = $_POST['albaranes'] === 'true';
    $notificaciones = $_POST['notificaciones'] === 'true';
    ...
    if ($notificaciones) {
        $pdo->exec("DELETE FROM notificaciones_enviadas");
        $eliminados[] = "notificaciones";
    }
    
    if ($albaranes) {
        $pdo->exec("DELETE FROM albaran_lineas");
        $pdo->exec("DELETE FROM albaranes");
        $pdo->exec("ALTER TABLE albaranes AUTO_INCREMENT = 1");
        updateConfig('siguiente_albaran', '1');
        $eliminados[] = "albaranes";
    }
    
    if ($servicios_cliente || $clientes) {
        $pdo->exec("DELETE FROM servicios_cliente");
        $pdo->exec("ALTER TABLE servicios_cliente AUTO_INCREMENT = 1");
        $eliminados[] = "servicios de clientes";
    }
    ...
    if ($servicios) {
        $pdo->exec("DELETE FROM tipos_servicios");
        $pdo->exec("ALTER TABLE tipos_servicios AUTO_INCREMENT = 1");
        $eliminados[] = "tipos de servicios";
    }
    
    $pdo->exec("SET FOREIGN_KEY_CHECKS = 1");
    
    $mensaje = "Eliminados: " . implode(", ", $eliminados) . ". Tablas limpiadas completamente.";
    ...
    break;
        ...
        case 'validar_borrado_cliente':
            ...
            // Obtener detalles de servicios si los hay
            $servicios_detalle = [];
        ...
        case 'edit_servicio_cliente':
            ...
            // Obtener días del nuevo tipo de vencimiento
            $stmt = $pdo->prepare("SELECT dias FROM tipos_vencimiento WHERE id = ?");
            $stmt->execute([$tipo_vencimiento_id]);
            $dias = $stmt->fetch()['dias'];
            
            // Calcular nueva fecha de vencimiento desde la fecha actual
            $nueva_fecha_vencimiento = date('Y-m-d', strtotime("+$dias days"));
            ...
            $stmt = $pdo->prepare("
                ...
                WHERE id = ?
            ");
            $stmt->execute([$tipo_vencimiento_id, $precio_personalizado, $observaciones, $nueva_fecha_vencimiento, $servicio_id]);
            
            $response['success'] = true;
            $response['message'] = 'Servicio actualizado y renovado correctamente';
            break;
        ...
        case 'renovar_servicio':
            ...
            // Calcular nueva fecha sumando los días al vencimiento actual
        ...
        case 'eliminar_albaran':
            ...
            // Eliminar primero las líneas del albarán
        ...
        case 'eliminar_albaranes_masivo':
            ...
            // Obtener cliente del primer servicio
        ...
        case 'get_filtros_data':
            // Obtener datos para filtros
            $tipos_servicios = $pdo->query("SELECT id, nombre FROM tipos_servicios WHERE activo = 1 ORDER BY nombre")->fetchAll();
            $estados_albaran = ['borrador', 'generado', 'enviado', 'pagado'];
            
            $response['success'] = true;
            $response['data'] = [
                'tipos_servicios' => $tipos_servicios,
                'estados_albaran' => $estados_albaran
            ];
            break;
